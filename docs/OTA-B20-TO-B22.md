...
## 8. 升级后恢复 MQTT 停用及认证开关

随后排查“部分设备未认证无法上网”时，发现恢复流程还遗漏了 MQTT 停用状态：
升级前 `zte_mqtt_sdk_st` 无自启链接，升级后恢复自启且实际连接厂商服务。
三个 `seecom_card_flag` 从升级前 `0/0/0` 变为 `0/1/0`，
本地二次认证链存在 DROP/HTTP 重定向；与此同时三条蜂窝出口分别直接访问 HTTPS 均成功。
因此这次直接拦截点在设备本地；未捕获到导致开关变化的云端命令，不能断言修改来源。
ICG 服务升级前已启用，不能将其也归为这次 OTA 新开启。

明确授权后，执行仓库 [vendor-control 工具](../vendor-control/)：通过 procd 停止并禁用 MQTT，
将三个认证开关持久化为 0，调用已静态核对的 B22 原厂关闭消息刷新运行态。
实测认证链及未认证客户端数归零，启动同步正常。工具不修改原厂认证成功记录，
不禁用 ICG 聚合，也不清空整体防火墙。未来 OTA 恢复清单应包含这一项并重新核对版本。

本次补充修复也经过真实重启：MQTT 仍停用、开关仍为 0/0/0、认证拦截规则与未认证计数均为 0，
原厂同步/注册、datad、Mihomo、ICG 和触屏进程正常。
